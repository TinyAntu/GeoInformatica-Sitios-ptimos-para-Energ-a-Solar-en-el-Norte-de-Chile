...
def generar_dataset_muestras(
    ...
    """
    Genera muestras positivas (plantas existentes) y un pool de negativos hard-mined.

    Flujo:
        1. Filtrar regiones y capas al norte de Chile
        2. Positivas: centroides de plantas existentes
        3. Negativos: puntos aleatorios reproducibles dentro de las regiones
        4. Hard Negative Mining: selección por GHI más alto
        5. Extracción conjunta de todos los rasters (GHI, slope, aspect, elev, dist_transmision)
        6. Relleno de NaN en positivas; filtro AHP en negativas
    """
    ...
    # --- AUDITORÍA DE LÍNEAS DE TRANSMISIÓN ---
    if 'REGION' in vectores['lineas'].columns:
        ...
        print("\n[ALERTA] La capa de líneas NO tiene una columna 'REGION'. Columnas disponibles:", vectores['lineas'].columns)
        lineas_norte = gpd.clip(vectores['lineas'], regiones_norte).to_crs(epsg=32718)

    # Limpieza de geometrías nulas
    ...
    # 3. Candidatos negativos aleatorios dentro de las regiones (reproducibles)
    bounds = regiones_norte.total_bounds
    n_neg_deseados = len(positivas) * ratio
    puntos_random = []

    # ESTÁNDAR ACADÉMICO: Definir un Espacio de Factibilidad Económica (Ej: Máximo 10 km de una línea)
    # Puedes ajustar este número (en metros) según los criterios de tu investigación
    radio_buffer_metros = 3000 
    ...
    # Usamos un contador de intentos para evitar bucles infinitos si el área es muy pequeña
    intentos = 0
    while len(puntos_random) < n_neg_deseados * 5 and intentos < n_neg_deseados * 100:
        intentos += 1
        x = rng.uniform(bounds[0], bounds[2])
        y = rng.uniform(bounds[1], bounds[3])
        pto = Point(x, y)
        
        # EL PUNTO DEBE CUMPLIR AMBAS CONDICIONES:
        # Estar dentro de Antofagasta/Atacama Y estar cerca de la red de transmisión
    ...
    # 4. Exclusión: buffer de 5 km alrededor de plantas existentes
    ...
    # 4b. Exclusión de zonas donde es físicamente o legalmente imposible instalar paneles:
    #     áreas pobladas, masas lacustres y áreas protegidas SNAP.
    ...
    for clave in claves_exclusion:
        ...
        try:
            ...
            print(f"  [AVISO] No se pudo recortar '{clave}' a la región: {e}. Usando capa completa.")
        if len(capa) > 0:
    ...
    if mascaras:
        ...
        print(f"  Candidatos eliminados por zonas de exclusión (poblados/lagos/SNAP): {antes - len(candidatos_neg)}")

    # 5. Hard Negative Mining: extraer GHI y seleccionar los de mayor irradiación
    ...
    n_pool = min(len(candidatos_neg), max(n_neg_deseados * 3, 10000))
    candidatos_top = candidatos_neg.sort_values('ghi', ascending=False).head(n_pool).copy()
    ...
    # GHI de candidatos_top ya fue extraído en el paso 5; se conserva para evitar doble lectura.
    # Las positivas no tienen ghi aún: su columna quedará NaN y se rellenará en el paso 8.
    ...
    # Validacion adicional: extraer distancia a líneas de transmisión si la capa existe y tiene geometría válida
    ...
    # 8. Rellenar NaN en positivas (ventana local → media global como fallback)
    ...
    # 9. Limpiar negativos: eliminar NaN y aplicar criterios AHP
    negativos_pre = negativos_pre.dropna(subset=['ghi', 'slope', 'aspect', 'elev'])
    ...
    #if len(lineas_norte) > 0:
    #  filtro_ahp &= negativos_pre['dist_transmision'] <= criterios['dist_max']
